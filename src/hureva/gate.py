...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("slug", help="feature slug under <specs_dir>/")
    ...
    spec_path = paths.spec(args.slug)
    spec = parse_spec(spec_path.read_text(encoding="utf-8"))

    result = evaluate_gate(
        spec, slug=args.slug, require_all_approvers=args.require_all_approvers
    )
